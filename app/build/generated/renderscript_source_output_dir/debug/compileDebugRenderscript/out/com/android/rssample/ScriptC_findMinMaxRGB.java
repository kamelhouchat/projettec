...
import com.android.rssample.findMinMaxRGBBitCode;
...
public class ScriptC_findMinMaxRGB extends ScriptC {
    ...
    // Constructor
    public  ScriptC_findMinMaxRGB(RenderScript rs) {
        super(rs,
              __rs_resource_name,
              findMinMaxRGBBitCode.getBitCode32(),
              findMinMaxRGBBitCode.getBitCode64());
        mRSLocal = rs;
    ...
    public static class resultArray3_int2 {
        ...
        private  resultArray3_int2(Allocation out) {
            mTempIns = null;
            mOut = out;
            mGotResult = false;
        }
    ...
    private final static int mExportReduceIdx_findMinAndMaxRGB = 0;
    // in1 = "in", flattened 4-vectors
    public resultArray3_int2 reduce_findMinAndMaxRGB(byte[] in1) {
        ...
        resultArray3_int2 result = reduce_findMinAndMaxRGB(ain1, null);
        result.mTempIns = new Allocation[]{ain1};
        return result;
    }

    // ain1 = "uchar4 in"
    public resultArray3_int2 reduce_findMinAndMaxRGB(Allocation ain1) {
        return reduce_findMinAndMaxRGB(ain1, null);
    }

    // ain1 = "uchar4 in"
    public resultArray3_int2 reduce_findMinAndMaxRGB(Allocation ain1, Script.LaunchOptions sc) {
        // check ain1
        if (!ain1.getType().getElement().isCompatible(__U8_4)) {
            throw new RSRuntimeException("Type mismatch with U8_4!");
        }
        Allocation aout = Allocation.createSized(mRSLocal, __I32_2, 3);
        aout.setAutoPadding(true);
        reduce(mExportReduceIdx_findMinAndMaxRGB, new Allocation[]{ain1}, aout, sc);
        return new resultArray3_int2(aout);
    }
